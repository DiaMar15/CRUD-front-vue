<template>
  <div class="peso">
    ...
    <button @click="procesarImagen" :disabled="!imagen || cargando">Procesar Imagen</button>
...
import { procesarImagenService, obtenerPesoService } from '@/services/entradaSalidaProductos';
import PesoTabla from '@/components/tablaHistorico.vue';

const peso = ref<number|null>(null);
const imagen = ref<File|null>(null);
const previewUrl = computed(() => imagen.value ? URL.createObjectURL(imagen.value) : '');
...
async function procesarImagen() {
  ...
  cargando.value = true;
  try {
    const texto = await procesarImagenService(imagen.value);
    textoReconocido.value = texto.match(/[\d.]+/g)?.join('') || '';
  } catch {
...
async function obtenerPeso() {
  ...
  try {
    ...
    peso.value = await obtenerPesoService(0, valorNumerico, unidad.value, categoria.value);
    alert(`Peso detectado: ${peso.value} ${unidad.value}`);
  } catch {
    alert('Ocurrió un error al obtener el peso.');
  }
}
</script>
